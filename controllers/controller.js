...
var dbx = new Dropbox({
    accessToken: 'am1YcJzsaBAAAAAAAAABKu1V85mn4PRi8iev8hJxu5A3T1DPq-fBoInqjlGUsShG'
});
var previewArg = {
    "path": "/test.docx"
}
...
module.exports.blogPageCtrl = function (req, res) {
    ...
    res.render("blogPage", {
        title: blogs[req.params.num].title,
        items: blogs[req.params.num],
        title: words
    });
};
...
module.exports.postEditBlog = function (req, res) {
    ...
    fs.writeFile(__dirname + "/../blog.json", JSON.stringify({
        "blogs": blogs
    }), function (err) {
        if (err) {
            console.log("ERROR: something is wrong");
